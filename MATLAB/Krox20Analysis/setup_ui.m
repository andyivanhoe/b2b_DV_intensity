function controls = setup_ui(data)
    ...
    controls.hfig = figure('Units', 'normalized', 'NumberTitle', 'off', ...
        ...
        'MenuBar', 'none', ...
        'ToolBar', 'figure');
    set(controls.hfig, 'Position', [0.05 0.05 0.8 0.8]);
...
%         'Position', [0.25, 0.05, 0.5, 0.025]);
    ...
    controls.hsetbotbut = uicontrol('Style', 'pushbutton', 'Parent', controls.hfig, ...
        'Units', 'normalized', ...
        'Position', [0.8, 1 - 0.05 - 2 * but_height, 0.15, but_height], ...
        'String', 'Set current Z plane as bottom of tissue', ...
        'Enable', 'off');
    
    % Set 80 micron high box over image
    controls.hplacebox = uicontrol('Style', 'pushbutton', 'Parent', controls.hfig, ...
        'Units', 'normalized', ...
        'Position', [0.8, 1 - 0.05 - 3 * but_height, 0.15, but_height], ...
        'String', 'Place box on image', 'Enable', 'off');
%     controls.hradiodrawtypegroup = uibuttongroup('Parent', controls.hfig, ...
%         'Units', 'normalized', ...
%         'Position', [0.8, 1 - 0.05 - 3 * but_height, 0.15, but_height], ...
%         'Title', 'Drawing method');
%     controls.hradiodrawtype(1) = uicontrol('Style', 'radio', 'Parent', controls.hradiodrawtypegroup, ...
%         'Units', 'normalized', ...
%         'Position', [0.05, 0, 0.5 - 0.05, 1], ...
%         'String', 'Box', ...
%         'HorizontalAlignment', 'center', ...
%         'Enable', 'off');
%     controls.hradiodrawtype(2) = uicontrol('Style', 'radio', 'Parent', controls.hradiodrawtypegroup, ...
%         'Units', 'normalized', ...
%         'Position', [0.05 + 0.5, 0, 0.5 - 0.05, 1], ...
%         'String', 'Edges', ...
%         'HorizontalAlignment', 'center', ...
%         'Enable', 'off');
    controls.hledgebut = uicontrol('Style', 'pushbutton', 'Parent', controls.hfig, ...
        'Units', 'normalized', 'Backgroundcolor', 'r', ...
        'Position', [0.79, 1 - 0.05 - 4 * but_height, 0.15, but_height], ...
        'String', 'Use current edge as L basal surface', ...
        'Enable', 'off', ...
        'Tag', 'L');
    controls.hredgebut = uicontrol('Style', 'pushbutton', 'Parent', controls.hfig, ...
        'Units', 'normalized', 'Backgroundcolor', 'g', ...
        'Position', [0.81, 1 - 0.05 - 5 * but_height, 0.15, but_height], ...
        'String', 'Use current edge as R basal surface', ...
        'Enable', 'off', ...
        'Tag', 'R');
    controls.hcalculate = uicontrol('Style', 'pushbutton', 'Parent', controls.hfig, ...
        'Units', 'normalized', 'Backgroundcolor', 'y', ...
        'Position', [0.81, 1 - 0.05 - 8 * but_height, 0.15, but_height], ...
        'String', 'Calculate basal-to-basal intensity', ...
        'Enable', 'on');
...
%         'Enable', 'off');
    controls.hshowchk = uicontrol('Style', 'checkbox', 'Parent', controls.hfig, ...
        'Units', 'normalized', ... 
        'Position', [0.8, 1 - 0.05 - (5 + 1.5)  * but_height, 0.15, but_height], ...
        'String', 'Display edges');
    controls.hhicontrast = uicontrol('Style', 'checkbox', 'Parent', controls.hfig, ...
        'Units', 'normalized', ... 
        'Position', [0.8 + 0.15/2, 1 - 0.05 - (5 + 1.5)  * but_height, 0.15/2, but_height], ...
        'String', 'High contrast?');
%     controls.hradiogroup = uibuttongroup('Parent', controls.hfig, ...
%         'Units', 'normalized', ...
%         'Position', [0.8, 1 - 0.05 - (5 + 3) * but_height, 0.15, 1.5 * but_height], ...
%         'Title', 'Fractional Z position relative to top/bottom (bottom = 0');
    
    % Radiogroup for selecting fraction Z position along DV axis
    controls.hradiogroup = uibuttongroup('Parent', controls.hfig, ...
        'Units', 'normalized', ...
        'Position', [0.025, 1 - 0.05 - 2 * but_height, 0.25, 1.5 * but_height], ...
        'Title', '% Along DV axis (ventral = 0)');
    
    controls.hfrm = uipanel('Parent', controls.hfig, ...
        'Units', 'normalized', ...
        'Position', [0.025, 0.35 - 1.5 * but_height, 0.2, 3 * but_height], ...
        'Title', 'Edges stored for this czi file');
...
%         'Units', 'normalized', ...
%         'Position', [0.5 + 0.05, 0, 0.5 - 0.05, 1], ...
...
%         'Enable', 'off');
    ...
    % Replaced: edge_string = {'L', 'R', 'M', 'Rh4', 'Rh6'};
    edge_string = {'L', 'R'};
    ...
    for zoind = 1:length(data.z_offsets)+1
        % check whether positions are in % or fractional... and turn into %
        if all(data.z_offsets <= 1)
            data.z_offsets = data.z_offsets * 100;
        end
        if zoind == 1
            hdum = uicontrol('Style', 'radio', 'Parent', controls.hradiogroup, ...
                'Units', 'normalized', ...
                'Position', [0.4, 0.9, 0.2, radio_height], ...
        ...
        elseif zoind ~= 1
            hdum = uicontrol('Style', 'radio', 'Parent', controls.hradiogroup, ...
                ...
                'String', [num2str(data.z_offsets(zoind - 1)) ' %'], ...
                'HorizontalAlignment', 'center', ...
                 'Enable', 'off');
...
%             hdum = uicontrol('Style', 'radio', 'Parent', controls.hradiogroup, ...
%                 'Units', 'normalized', ...
%                 'Position', [0.2, 0.9 - (zoind - 1) * radio_height, 0.2, radio_height], ...
%                 'String', [num2str(data.z_offsets(zoind - 1)) ' %'], ...
%                 'HorizontalAlignment', 'left', ...
%                 'Enable', 'off');
%             controls.hzradios = [controls.hzradios; hdum];
%          end
        set(controls.hzradios, 'Value', 0);
        ...
        for eind = 1:(length(edge_string) + 1)
            ...
            % Put L or R as title of checkbox columns
            if (zoind == 1) && (eind > 1)
            ...
            % Put 6 checkboxes in panel, 3 in each column and give them
            % Tags
