...
% %% Try to edit to remove timepoint references (and maybe rhombomeres too)
% 
% % check whether current z plane exists in array of stored edges...
%     edg_str = {'L', 'R', 'M'};
%     if ~isempty(data.edges)
...
%         
%             if any(zs == z)
%                  data.edges(zs == z).timestamp = data.timestamps(t);
%                 if ~( strcmp(edg, 'Rh4') || strcmp(edg, 'Rh6') )
%                     if isa(data.current_edge, 'impoly')
%                         data.current_edge = data.current_edge.getPosition;
%                     end
%                     data.edges(zs == z).(edg) = data.current_edge;
%                     data.edges(zs == z).edgeValidity(strcmp(edg_str, edg),:) = ...
%                         check_edge_validity(data, data.edges(zs == z));
% %                     if any(data.edges((zs == z).edgeValidity(strcmp(edg_str, edg),:))
% %                         data.edges(zs == z).(edg) = data.current_edge;
% %                     end
%                 else
...
%                 if ~( strcmp(edg, 'Rh4') || strcmp(edg, 'Rh6') )
%                     if any(data.edges(zs == z).edgeValidity(strcmp(edg_str, edg),:))
%                         data.edges(zs == z).(['hl' edg]) = line(data.current_edge(:,1), ...
%                             data.current_edge(:,2), ...
%                             'Color', 'r', ...
%                             'Visible', vis);
%                     end
%                 else
%                     data.edges(zs == z).(['hl' edg]) = patch(data.current_edge(:,1), ...
%                         data.current_edge(:,2), ...
%                         'r', ...
%                         'EdgeColor', 'r', ...
%                         'LineWidth', 2, ...
